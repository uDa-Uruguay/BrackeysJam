...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Movement()
    {
        ...
        {
            verticalSpeed = 0;
            if (Input.GetKey(KeyCode.Space)) verticalSpeed = Mathf.Sqrt(_jumpHeight * -2f * -gravity);
        } else  verticalSpeed -= gravity * Time.deltaTime; // Applies gravity to vertical speed.
        ...
        _characterController.Move(moveDir * _movementSpeed * Time.deltaTime);
    }

    public void ChangeMovementState()
    {
        if (enableMovement) enableMovement = false;
